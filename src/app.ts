...
async function testing() {
    ...
    const data = exampleGroup.simsGenerator.createMatching(exampleMatching);
    console.log(data);

    const sims = exampleGroup.simsGenerator.generateSims(data, SERVER_SIMS_KEY, iv);
    console.log(sims);

    const receivedData = exampleGroup.simsGenerator.extractMatching(sims, SERVER_SIMS_KEY, iv);
